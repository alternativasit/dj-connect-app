...
export default async function EventHomePage({ params, searchParams }: { params: { eventSlug: string }; searchParams?: { preview?: string } }) {
  ...
  const features = [
    { title: "Request a Song", text: "Send Request and track My Requests.", href: base + "/request-song" + previewQuery, icon: <Music2 size={22} /> },
    { title: "Live Polls", text: "Vote in genre and song polls in real time.", href: base + "/polls" + previewQuery, icon: <Radio size={22} /> },
    { title: "Drinks Menu", text: "Browse cocktails, beer, shots and specials.", href: base + "/drinks" + previewQuery, icon: <GlassWater size={22} /> },
    { title: "Event Promos", text: "See offers from the venue and sponsors.", href: base + "/promos" + previewQuery, icon: <Gift size={22} /> },
    { title: "Tip the DJ", text: "Open PayPal, Venmo, Cash App and more.", href: base + "/tip" + previewQuery, icon: <HeartHandshake size={22} /> },
    { title: "DJ Profile", text: "Follow, book and see packages.", href: base + "/dj" + previewQuery, icon: <UserRound size={22} /> }
  ];
  ...
  return (
    <AppShell
      ...
      subtitle={bundle.venue?.name || bundle.dj.name}
    ...
    >
      <div className="space-y-5">
        ...
        <DarkCard className="relative overflow-hidden">
          <div className="relative z-10">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              ...
              <Link href={base + "/request-song" + previewQuery} className="rounded-2xl border border-line bg-surface2 px-5 py-3 text-sm font-semibold text-white">Request a Song</Link>
            </div>
          </div>
        </DarkCard>
        ...
        <RequestedSongsFeed eventId={bundle.event.id} requestHref={base + "/request-song" + previewQuery} initialRequests={bundle.songRequests} />

        <div className="grid gap-4 lg:grid-cols-[1fr_0.9fr]">
          <DJProfileCard dj={bundle.dj} href={base + "/dj" + previewQuery} />
          <QRCard url={url} />
        </div>
